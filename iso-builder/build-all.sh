...
cleanup() {
  ...
  if [ -n "$ISO_MOUNT" ] && [ -d "$ISO_MOUNT" ]; then
    ...
    sudo rm -rf "$ISO_MOUNT" 2>/dev/null || true
  fi
...
BASE_ISO=$(find void-mklive -name "void-base-x86_64-*.iso" -type f | head -1)
if [ -z "$BASE_ISO" ]; then
...
DESIRED_SIZE=$((20 * 1024 * 1024 * 1024))
